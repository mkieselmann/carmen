...
function nearestPoints(source, lon, lat, scoreFilter, callback) {
    ...
    function afterQuery(err, results) {
        ...
        for (let results_it = 0; results_it < results.length; results_it++) {
            ...
            hit.tmpid = (source.idx * mp24) + termops.feature(attr.id);
...
function processVtQueryResults(results, source, lon, lat,  full, matched, language, scoreFilter,reverseMode, routing, callback) {
    ...
    for (let i = 0; i < results.length; i++) {
        ...
        const tmpid = (source.idx * mp24) + termops.feature(attr.id);
...
function lightFeature(source, feat, callback) {
    ...
    loaded.properties['carmen:tmpid'] = (source.idx * mp24) + termops.feature(feat.id);
